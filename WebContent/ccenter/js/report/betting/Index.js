...
define(['common/BaseListPage'], function (BaseListPage) {
    return BaseListPage.extend({
        ...
        bindEvent: function () {
            ...
            $(this.formSelector).on("blur", "input[name='search.siteId']", function () {
                _this.searchSite();
            });
        },
         ...
         * @param option
         */
        changeMaster: function (e, option) {
            $(".siteNames").hide();
            $("[selectdiv='siteName" + e.value + "']").show();
        },
        /**
         * 更换站点
         * @param e
         * @param option
         */
        changeSite: function (e, option) {
            $("input[name='search.siteId']").val(e.key);
        },

        searchSite: function () {
            var $siteId = $("input[name='search.siteId']");
            var siteId = $siteId.val();
            if (siteId && $siteId.valid()) {
                window.top.topPage.ajax({
                    url: root + "/report/gameTransaction/searchSiteId.html",
                    data: {"siteId": siteId},
                    dataType: "json",
                    success: function (data) {
                        if (data && data.username && data.siteId) {
                            select.setValue($("[selectdiv = mastername]"), data.username);
                            $(".siteNames").hide();
                            select.setValue($("[selectdiv = siteName" + data.username + "]"), data.siteId);
                            $("[selectdiv = siteName" + data.username + "]").show();
                        } else {
                            window.top.topPage.showWarningMessage("找不到站点，请重新输入！");
                        }
                    },
                    error: function (data) {
                        console.log(data);
        ...
        apiInit: function (e, obj) {
            ...
            if (obj) {
                $("#chooseApi").children("div.input-group").find("span[prompt=prompt]").text("已选全部");
            } else {
                $("#chooseApi").children("div.input-group").find("span[prompt=prompt]").text("请选择");
            }
            $(e.currentTarget).unlock();
        },
        /**
        ...
        chooseApi: function (e, option) {
            ...
            if (len > 0) {
                $("#chooseApi span[prompt=prompt]").text("已选中" + len + "项");
                $("#selectGame").text(html);
            } else {
                $("#chooseApi span[prompt=prompt]").text("请选择");
                $("#selectGame").text(window.top.message.report['operate.list.all']);
            }
        },
        /**
        ...
        reWrite: function () {
            ...
            if (selectedJson != '') {
                ...
                $(this.formSelector + " #chooseGameType .input-group").find("span[prompt=prompt]").text("已选择" + count + "项");
        ...
        apiVal: function (e) {
            ...
            if (arr && arr.length > 0) {
                ...
                if (selectGame.length > 0) {
                    ...
                    $("#chooseGameType").children(".input-group").find("span[prompt='prompt']").text("已选中" + len + "项");
                }
            } else {
