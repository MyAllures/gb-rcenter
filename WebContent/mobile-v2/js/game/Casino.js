...
define(['site/include/BaseIndex'], function(BaseIndex) {
    return BaseIndex.extend({
        ...
        onPageLoad: function () {
            ...
            var _this = this;
            _this.initPage();
            var apiId = $('#casino-id').val();
            if (typeof apiId !== 'undefined') {
                _this.loadData($('#casino-id').val());
            }
        },
        initPage: function () {
            var _this = this;
            mui(".electronic-type").scroll();
            mui('.mui-casino').on('tap', '[data-casino-id]', function() {
                $('input#game-name').val('');
                var id = $(this).data('casino-id');
            ...
            mui('.electronic-srch').on('tap', '#search-game', function() {
                ...
                mui.ajax(root + '/game/getGameByApiId.html', {
                    type: 'GET',
                    data: {'search.apiId': apiId, 'search.apiTypeId': 2, 'search.name': gameName, 'paging.pageNumber': pageNumber},
                    headers: {
                    ...
                    success: function(data) {
                        setTimeout(function() {
                            ...
                            $('div.casino-content-' + apiId).html(data);
        ...
        loadData: function(apiId) {
            ...
            mui.ajax(root + '/game/getGameByApiId.html', {
                type: 'GET',
                data: {'search.apiId': apiId, 'search.apiTypeId': 2, 'paging.pageNumber': pageNumber},
                headers: {
                    'Content-Type':'application/json',
                    'Soul-Requested-With':'XMLHttpRequest'
                },
