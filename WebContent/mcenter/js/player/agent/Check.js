...
define(['common/BaseEditPage'], function (BaseEditPage) {
    ...
    return BaseEditPage.extend({
        ...
        myValidateForm:function(e,option){
            ...
            if(val==4){
                var tt = option.tt;
                var msg = window.top.message.player_auto['确认审核通过吗'];//确认审核不通过吗？
                if(tt=='failure'){
                    msg = window.top.message.player_auto['确认审核不通过吗'];
                }
                ...
                window.top.topPage.showErrorMessage(window.top.message.player_auto['该条申请已审核']);
            }
            return false;
        },
