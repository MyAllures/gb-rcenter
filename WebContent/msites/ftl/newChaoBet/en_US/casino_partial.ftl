...
<div class="casino-sorts m-t-md">
    ...
    <#list data.gameTagsOfApiType as tag>
        ...
        <li class="<#if !data.gameSearch.gameTag?has_content>active</#if>"><a class="_vr_getGames" href="javascript:void(0)" data-href="casino_partial.html?apiType=${apiType}&apiId=${data.gameSearch.apiId?default('')}">All game</a></li>
    </ul>
    <div class="search-box header-login search">
        ...
        <input type="text" name="gameName" class="form-control input-sm pull-left" placeholder="Please enter the name of the game">
        <a href="javascript:void(0)" class="btn btn-search pull-left _vr_buttonSubmit">Search</a>
    </div>
</div>
<!-- game-list -->
<div class="game-list">
                ...
                <figure class="game-item" data_id="001">
                    ...
                    <figcaption class="title">
                        ...
                        <a class="_vr_mt_check _vr_mt_slogan btn-play" data-api="${game.apiId?string.computer}" data-game-name="${data.siteGameI18ns[game.gameId?string.computer].name}" data-game-code="<#if data.gameMapById[game.gameId?string.computer]?has_content>${data.gameMapById[game.gameId?string.computer].code}</#if>" href="javascript:void(0)"
                           startTime="<#if data.gameMapById[game.gameId?string.computer].maintainStartTime?has_content>${data.gameMapById[game.gameId?string.computer].maintainStartTime?long?string.computer}</#if>"
                           endTime="<#if data.gameMapById[game.gameId?string.computer].maintainEndTime?has_content>${data.gameMapById[game.gameId?string.computer].maintainEndTime?long?string.computer}</#if>">Start game</a>
                    </figcaption>
                </figure>
            </div>
        </#list>
    <#else>
        <div class="col-1-1" style="text-align: center;line-height: 251px"><span class="text-center text-warning">No content！</span></div>
    </#if>
    </div>
    <!-- Pagination -->
    <div class="row">
