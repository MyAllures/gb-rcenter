...
<script>
    ...
    function promoCheck() {
        ...
        $("._vr_promo_check").each(function () {
            ...
            if (nowTime < sTime) {
                ...
                $this.find("._vr_promo_join").removeClass(typeof oldClass=="undefined"?"":oldClass).addClass(typeof newClass=="undefined"?"":newClass).attr("onclick", "").text("over");
    ...
    function changeApplyStatus() {
        ...
        backwaterObj.each(function (i, obj) {
            $(obj).find("._vr_promo_join").text("Joining");
        });
        $.ajax({
            ...
            if (($(actObj).data("code") == "first_deposit" || $(actObj).data("code") == "deposit_send") && flag) {
                if (data.length < 1) {
                    if ($(actObj).data("rankId") === "all") {
                        $(actObj).find("._vr_promo_join").removeClass(oldClass).addClass(newClass + " disabled").text("Deposit when apply");
                    } else {
