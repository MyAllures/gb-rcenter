...
<script>
    ...
    function createDesktop() {
        ...
        var sName = "<#if data.siteInfo.title?exists>${data.siteInfo.title}</#if>A shortcut";
        try {
            ...
            if (!fso.FileExists(folderPath + "//" + sName + ".lnk")) {
                ...
                shortLink.Save();
                alert('Desktop shortcuts were created successfully！');
            }
        } catch (e) {
    ...
    function AddFavorite(url,title) {
        ...
        try {
            ...
            try {
                ...
                alert("Sorry, the browser you are using can not do this. Please use Ctrl + D to add it");
            }
        }
    }
    ...
    * */
    ...
    function alert(message){
        layerDialogNormal(message,'Alert','layui-layer-brand',['360px']);
    }
    ...
    <#if imgSrc??>
        if(!localStorage.getItem("${updateTime}"+"-close-home-dialog")){// 判读缓存里是否关闭了首页弹窗
            ...
            <#elseif content?has_content>
                layerDialogIndex('<div style="padding:10px;">${content?replace('\n','')?replace('\r','')}</div><div class="checkbox-wrap"><input type="checkbox" id="home-dialog-checkbox" />After closing, no longer display the popover advertisement.</div>','${imgTitl}','layui-layer-brand',['600px'],'','r',true);
    ...
    function handleMt($handle){
        ...
        $handle.find("._vr_mt_slogan").text("In maintenance");
        $handle.find("._vr_mt_ptSlogan").text("(In maintenance)");
    ...
    function maintainInfo(st, et,apiId,gameName){
        ...
        if(gameName == 'undefined' || typeof gameName == 'undefined'){
            BootstrapDialog.alert({
                message:"<div>Dear customer Hello:</div><div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+apiName+" Platform will be maintained at "+transTimeZone(sessionStorage.getItem("timezone"))+" "+sTime+" - "+eTime+"maintenance time is subject to change without notice. To bring you the inconvenience, please understand！</div>",
                title:'Prompt information',
                type: BootstrapDialog.TYPE_PRIMARY
            });
        }else{
            BootstrapDialog.alert({
                message:"<div>Dear customer Hello:</div><div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+gameName+" Game will be maintained at "+transTimeZone(sessionStorage.getItem("timezone"))+" "+sTime+" - "+eTime+"maintenance time is subject to change without notice. To bring you the inconvenience, please understand！</div>",
                title:'Prompt information',
                type: BootstrapDialog.TYPE_PRIMARY
            });
        }
    ...
    function transTimeZone(timezone){
        ...
        switch (tz){
            case "GMT+08:00":
                trans = "Beijing time";break;
            case "GMT-04:00":
                trans = "East time";break;
        }
    ...
    function forgetUsername(){
        layerDialogForgetAccount('<div style="font-size:  16px;font-weight: bold;color:  #000;margin-bottom: 5px;">Forget account?Please contact the online customer service </div><div>They will give your account back after confirming your user information</div>','Find Account','layui-layer-brand',['400px','210px'],false,false);
    }
    ...
    function balanceRefresh(){
        ...
        $("._vr_balanceBox").mouseenter(function(){
            ...
            if(isAutoPay == 'true') {
                $("li[name='apiPreNode']").html(' <a href="javascript:" onclick="recoveryApi()"><span class="icon-back-m"></span>One key recovery</a>');
            } else {
                $("li[name='apiPreNode']").html('<a href="javascript:" onclick="getApiBalance()"><span class="pull-right gui gui-refresh"></span>Refresh balance</a>')
            }
    ...
    function getNotAutoPayApiBalance() {
        ...
        $.ajax({
            ...
            dataType:"JSON",
            beforeSend:function(){
                $("li[name=apiPreNode]").html(' <a href="javascript:" onclick="getApiBalance()"><span class="pull-right gui gui-refresh"></span>Refresh the balance</a>');
                    ...
                    loginObj.getLoginPopup(function (logined) {
                        if(logined){
                            if(apiTypeId == "3" && apiId=="19"){
                                window.open("https://mkt.fafauuweb.com?lang=cs");
                            }else{
                                currentPage(apiId);
                            }
                        }
  ...
  * */
    function changeBalanceHide(bol){
        if(typeof bol === 'undefined'){
            ...
            $('span.eye').removeClass('gui-eye').addClass('gui-eye-slash').attr('title', 'Show balance');
            $('li.f_refresh').hide();
            ...
            $('span.eye').removeClass('gui-eye-slash').addClass('gui-eye').attr('title', 'Hide balance');
            $('li.f_refresh').show();
        ...
        getLoginPopup:function (callback){
            ...
            loginObj.loginDialog = layer.open({
                ...
                success: function(layer){
                    ...
                    $("#loginForm .code._vr_captcha_box").after('<a href="javascript:void(0);" class="layui-layer-btn0 btn-login dialog_login">Login</a>');
                    // 提示框类型
                    ...
                    $('#loginForm .dialog_login').on("click",function(e){
                        ...
                        if(loginText==""){
                            ...
                            $(e.target).text("Logging in").css("pointer-events","none");
                        }
                        var $this = $(this);
    ...
    $("._vr_login").on("click", function (e) {
        ...
        if(loginText==""){
            ...
            $(e.target).text("Logging").css("pointer-events","none");
        }
        var $this = $(this);
...
//                    getApiBalance();
    ...
    function recoveryApi(obj) {
        var isAutoPay = getCookie("isAutoPay");
        if(isAutoPay != 'true') {
            dialogMsg("The current settings are free, can not be recycled！");
            return;
        }
            ...
            success:function(data){
                if (data) {
                    if (data.msg) {
                        ...
                        dialogMsg("recycling all API funds ,please wait！");
                        autoGetApiBalance(obj);
                    } else if (data.resultStatus) {
                        if (data.resultStatus == 'SUCCESS') {
                            dialogMsg("Recycling successful, please check the wallet balance!");
                            autoGetApiBalance(obj);
                        } else if (data.resultCode == 1) {
                            dialogMsg("Recovery failure, failure status code[" + data.resultStatus + "],please try again later！");
                        } else {
                            dialogMsg("In the recycling, please check back later！");
                            autoGetApiBalance(obj);
                        }
                    } else {
                        dialogMsg("In the recycling, please check back later!");
                    ...
                    dialogMsg("The system is busy, please try again later！");
                }
            },
            error:function(error){
                console.log(error);
    ...
    function dialogMsg(msg) {
        layerDialogNormal(msg,'Prompt','layui-layer-brand',['360px']);
    }
    ...
    function layerDialogIndex(imgSrc,title,skin,area,transparent,btnAlign,txtDialog){
        ...
        if(!!txtDialog){
            ...
            <#if link??>
                ...
                content = $("<div></div>").append($(img)).append('<div class="checkbox-wrap"><input type="checkbox" id="home-dialog-checkbox" >After closing, no longer display the popover advertisement. <div>');
            <#else>
                content = $("<div></div>").append($(img)).append('<div class="checkbox-wrap"><input type="checkbox" id="home-dialog-checkbox" >After closing, no longer display the popover advertisement.<div>');
            </#if>
                layer.open({
    ...
    function layerDialogDownload(){
        ...
        layer.tab({
            ...
            move:".layui-layer-title",
            tab: [{
                title: '<div class="tit-wrap"><div class="tit">Mobile App Download</div><div class="sub-tit">For Android and Ios</div></div>',
                content: $("#download-mobile").html()
            }, {
                title: '<div class="tit-wrap"><div class="tit">API App Download</div><div class="sub-tit">Desktop Version，Plenty App version</div></div>',
    ...
    function layerDialogForgetAccount(content,title,skin,area,btnRound,btnBorder){
        ...
        layer.open({
            ...
            skin:skin,
            area:area,
            btn:["Contact Customer Service","Cancel],
            success: function(layer){
                // 重写关闭按钮
                $(layer).find('.layui-layer-setwin').html('<a class="layui-layer-close" href="javascript:;">	&times;</a>');
                // 提示框类型
...
<div id="login-dialog" style="display:none;">
    <form  id="loginForm" method="post">
        ...
        <div class="form-group account">
            ...
            <div class="tip" style="display: none;">Please enter your account！</div>
        </div>
        <div class="form-group password">
            <input type="password" class="form-control" placeholder="Password" name="password" />
            <div class="tip" style="display: none;">Please enter your password！</div>
        </div>
        <div class="form-group code _vr_captcha_box" style="display: none;">
            <input type="text" class="form-control" placeholder="Verification code" name="captcha" maxlength="4" />
            <img class="_vr_captcha_code test" data-code="loginDialog">
            <div class="tip" style="display: none;">Please enter verification code！</div>
        </div>
        <a href="javascript:void(0);" class="btn-login dialog_login">Login</a>
        <a  target="_blank" href="commonPage/forgetPwd.html" class="forget-pas">Forget Password？</a>
        <span style="font-size: 14px;color: #666;margin-right: 10px;"> Don't have account？</span>
...
<div id="download-mobile" style="display:none;">
    <div class="code_item ios">
        ...
        <div class="tit_d">iOS APP Download</div>
        <p>Scan QR code with iPhone browser <br />to Download APP</p>
    </div>
    <div class="code_item android">
        <div class="qrcode qr2" id="code_android">
        </div>
        <div class="tit_d">Android APP Download</div>
        <p>Scan QR code with Android browser， <br />to Download APP</p>
    </div>
</div>
        ...
        <ul class="api-list">
            <li>
                <div class="api-name ag">
                    <div class="nam">AG Clients</div>
                    <div class="tip">Login Please add a prefix dawoo_， <br />e.g：dawoo_user01</div>
                </div>
                <div class="download-btn-group">
                    ...
                    <a href="" class="btn-pc">PC Clients</a>
                </div>
            </li>
            <li>
                <div class="api-name pt">
                    <div class="nam">PT Clients</div>
                    <div class="tip">Login Please add a prefix dawoo_， <br />e.g：dawoo_user01</div>
                </div>
                <div class="download-btn-group">
                    <a href="" class="btn-android">Android APP
            ...
            <li>
                <div class="api-name bb">
                    <div class="nam">BBIN Clients</div>
                    <div class="tip">Login Please add a prefix dawoo_， <br />e.g：dawoo_user01</div>
                </div>
                <div class="download-btn-group">
                    <a href="" class="btn-android">Android APP
...
<div id="index-modal-content" style="display: none;">
    ...
    <div class="checkbox-wrap">
        <input type="checkbox"  />After closing, this popup window advertisement no longer appears
    </div>
</div>
<!--首页透明弹窗内容&ndash;&gt;
