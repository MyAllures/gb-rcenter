...
var KaelQrcode = function () {
    var a, b, c, d = {}, e = {}, f = {
        colorHex: function (a) {
            var b = /^#([0-9a-fA-f]{3}|[0-9a-fA-f]{6})$/, c = a;
            if (/^(rgb|RGB)/.test(c)) {
                for (var d = c.replace(/(?:\(|\)|rgb|RGB)*/g, "").split(","), e = "#", f = 0; f < d.length; f++) {
                    var g = Number(d[f]).toString(16);
                    "0" === g && (g += g), e += g
                }
                return 7 !== e.length && (e = c), e
            }
            if (!b.test(c))return c;
            var h = c.replace(/#/, "").split("");
            if (6 === h.length)return c;
            if (3 === h.length) {
                for (var i = "#", f = 0; f < h.length; f += 1)i += h[f] + h[f];
                return i
            }
        }, colorRgb: function (a) {
            var b = /^#([0-9a-fA-f]{3}|[0-9a-fA-f]{6})$/, c = a.toLowerCase();
            if (c && b.test(c)) {
                if (4 === c.length) {
                    for (var d = "#", e = 1; 4 > e; e += 1)d += c.slice(e, e + 1).concat(c.slice(e, e + 1));
                    c = d
                }
                for (var f = [], e = 1; 7 > e; e += 2)f.push(parseInt("0x" + c.slice(e, e + 2)));
                return "RGB(" + f.join(",") + ")"
            }
            return c
        }, changeRGB: function (a, b) {
            function d(a) {
                var c = a.match(/\([^\)]+\)/g)[0];
                c = c.substr(1, c.length - 2);
                for (var d = c.split(","), e = 0, f = d.length; f > e; e++)d[e] = parseInt(d[e]) + b, d[e] < 0 && (d[e] = 0);
                return "rgb(" + d.join(",") + ")"
            }

            if ("object" == typeof a) {
                var g = c.createLinearGradient(0, 0, 0, e.width);
                for (var h in a) {
                    var i = a[h];
                    i = d(i), g.addColorStop(h, i)
                }
                return g
            }
            return "string" == typeof a ? d(a.indexOf("#") < 0 ? a : f.colorRgb(a)) : void 0
        }
    }, g = function () {
        b = document.createElement("canvas"), b.width = e.width, b.height = e.height, c = b.getContext("2d")
    }, h = function () {
        a = new QRCode(e.typeNumber, e.correctLevel), a.addData(e.text), a.make()
    }, i = function (b, c) {
        var d = a.getModuleCount(), e = {l: !1, r: !1, t: !1, b: !1, row: b, col: c};
        return 0 != c && a.isDark(b, c - 1) && (e.l = !0), c != d - 1 && a.isDark(b, c + 1) && (e.r = !0), 0 != b && a.isDark(b - 1, c) && (e.t = !0), b != d - 1 && a.isDark(b + 1, c) && (e.b = !0), 8 == b && 6 == c && console.log(e), e
    }, j = function () {
        var a = new Image;
        a.src = e.img.src;
        var b = e.width / 5, d = e.width / 10 * 4, f = e.width / 120;
        c.strokeStyle = e.img.border || "#fff", c.lineWidth = e.width / 40, c.globalAlpha = 1, c.lineCap = "round", c.lineJoin = "round", c.beginPath(), c.moveTo(d - f, d - f), c.lineTo(d + b + f, d - f), c.lineTo(d + b + f, d + b + f), c.lineTo(d - f, d + b + f), c.lineTo(d - f, d - f), c.stroke(), c.closePath(), a.onload = function () {
            c.drawImage(a, d, d, b, b), c.beginPath()
        }
    }, k = function (a, b) {
        var g = f.changeRGB(d.color, -20), h = f.changeRGB(d.pointColor, -20);
        a.l || a.r || a.t || a.b ? b ? c.fillStyle = g : (c.strokeStyle = e.color, c.fillStyle = e.color) : b ? c.fillStyle = h : (c.strokeStyle = e.pointColor, c.fillStyle = e.pointColor)
    }, l = function () {
        c.fillStyle = e.background, c.fillRect(0, 0, e.width, e.height)
    }, m = function (b, g, h) {
        {
            var j = g.row, l = g.col, m = g.tileW, n = g.tileH, o = g.w, p = g.h, q = f.changeRGB(d.color, -20);
            f.changeRGB(d.pointColor, -20)
        }
        if ("round" == b) {
            var r = a.isDark(j, l);
            if (r) {
                {
                    var s = i(j, l);
                    e.width / 5, e.width / 10 * 4
                }
                if (h)c.fillStyle = q, c.strokeStyle = q; else {
                    var o = (l + 1) * m - l * m, p = (j + 1) * m - j * m;
                    e.pointColor ? k(s, h) : (c.fillStyle = e.color, c.strokeStyle = e.color)
                }
                c.lineWidth = 2, c.globalAlpha = 1, c.lineCap = "round", c.lineJoin = "round", c.beginPath();
                var t = Math.round(l * m), u = Math.round(j * n), v = o / 2;
                s.l || s.t ? c.moveTo(t, u) : c.moveTo(t + v, u), s.r || s.t ? c.lineTo(t + o, u) : c.arcTo(t + o, u, t + o, u + p, v), s.r || s.b ? c.lineTo(t + o, u + p) : c.arcTo(t + o, u + p, t, u + p, v), s.l || s.b ? c.lineTo(t, u + p) : c.arcTo(t, u + p, t, u, v), s.l || s.t ? c.lineTo(t, u) : c.arcTo(t, u, t + o, u, v), c.stroke(), c.fill(), c.closePath()
            }
        } else {
            var r = a.isDark(j, l);
            r && (e.pointColor ? k(i(j, l), h) : c.fillStyle = h ? q : e.color, c.fillRect(Math.round(l * m), Math.round(j * n), o, p))
        }
    }, n = function () {
        var c = e.width / a.getModuleCount(), d = e.height / a.getModuleCount();
        if (l(), e.shadow)for (var f = 0; f < a.getModuleCount(); f++)for (var g = 0; g < a.getModuleCount(); g++) {
            var h = Math.ceil((g + 1) * c) - Math.floor(g * c), i = Math.ceil((f + 1) * c) - Math.floor(f * c), k = e.width / 150;
            m(e.type, {row: f, col: g, tileW: c, tileH: d, w: h + k, h: i + k}, !0)
        }
        for (var f = 0; f < a.getModuleCount(); f++)for (var g = 0; g < a.getModuleCount(); g++) {
            var h = Math.ceil((g + 1) * c) - Math.floor(g * c), i = Math.ceil((f + 1) * c) - Math.floor(f * c);
            m(e.type, {row: f, col: g, tileW: c, tileH: d, w: h, h: i})
        }
        return e.img && j(), b
    }, o = function (a, b) {
        if (d = b, "string" == typeof b ? e = b = {text: b} : e.text = b.text || "litten", e.width = b.size || 150, e.height = b.size || 150, e.shadow = b.shadow || !1, g(), e.typeNumber = b.typeNumber || -1, e.correctLevel = QRErrorCorrectLevel.H, e.pointColor = b.pointColor || null, e.type = b.type || "base", e.img = "string" == typeof b.img ? {src: b.img} : b.img, b.background) {
            var f = typeof b.background;
            if ("string" == f)e.background = b.background; else if ("object" == f) {
                var i = c.createLinearGradient(0, 0, 0, e.width);
                for (var j in b.background)i.addColorStop(parseInt(j), b.background[j]);
                e.background = i
            } else e.background = "#fff"
        } else e.background = "#fff";
        if (b.color) {
            var f = typeof b.color;
            if ("string" == f)e.color = b.color; else if ("object" == f) {
                var i = c.createLinearGradient(0, 0, 0, e.width);
                for (var j in b.color)i.addColorStop(j, b.color[j]);
                e.color = i
            } else e.color = "#000"
        } else e.color = "#000";
        h(), a.appendChild(n())
    };
    return {init: o, config: e}
};
QR8bitByte.prototype = {
    ...
    for (var c = 0, d = 0, e = 0, f = new Array(b.length), g = new Array(b.length), h = 0; h < b.length; h++) {
        ...
        c += i;
        var l = QRUtil.getErrorCorrectPolynomial(j), m = new QRPolynomial(f[h], l.getLength() - 1), n = m.mod(l);
