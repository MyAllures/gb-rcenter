<script>
    ...
    $(document).on("click","._game_open",function(e){
        ...
        if(Number(gameLine)>0){
            ...
            $(".game-info .game-item .g_tx").text(gameLine+"ライン");
        }
        if(apiId=="10"){
        ...
        if(fullscreen=="fullscreen"){
            ...
            // 提示使用左上角的退出来退出全屏
            layer.msg('alt + f4を押し続けると、フルスクリーンウィンドウが閉じます',{time:3000});
        }
    ...
    function showRecharge(data,apiId,gameCode,apiTypeId){
        var dialog = layer.open({
            ...
            '<div style="width: 400px;margin: 0 auto 10px;"><button class="btn btn-success btn-block" type="button" onclick="autoPayLogin('+apiId+','+"'"+gameCode+"'"+','+apiTypeId+')"><span class="gui gui-share"></span> ゲームに入る</button></div>',
            title:'バランスを思い出させる',
            area:['600px','285px'],
            skin:'layui-layer-brand',
            success: function(layer){
    ...
    function confirmTransction(apiId,gameCode,apiTypeId){
        ...
        $.ajax({
            ...
            dataType:"JSON",
            beforeSend:function(){
                $("#confirm-btn").html('<span class="loading gui gui-spinner fa-pulse"></span> 転送');
            },
            success:function(data){
                if (data.isFreeze == true) {
                    $("#confirm-btn").html('<span class="gui gui-check-square-o"></span> 口座残高が凍結');
                    alert("口座残高が凍結");
                    return;
                } else if (data.apiStatus == false) {
                    $("#confirm-btn").html('<span class="gui gui-check-square-o"></span> APIのメンテナンスまたは非アクティブ化');
                    alert("このapiゲームは維持されている（または無効になっている）ため、転送することはできません！");
                } else if (data.state == false && data.hasAccount == false) {
                    $("#token").val(data.token);
                    $("#confirm-btn").html('<span class="gui gui-check-square-o"></span> 転送を確認する');
                    alert("この時点でApiを転送することはできません。後でもう一度お試しください！");
                    setButtonStatus();
                    ...
                    if (data.resultCode == 0) {
                        alert("転送の成功");
                        $("#confirm-btn").html('<span class="loading gui gui-check"></span> 転送完了');
                        enterToGame(apiId,gameCode,apiTypeId);
                    }else {//转账不成功或待确认
                        $("#token").val(data.token);
                        $("#confirm-btn").html('<span class="gui gui-check-square-o"></span> 転送を確認する');
            ...
            error:function(XMLHttpRequest, textStatus, errorThrown) {
                if(textStatus=="608"){
                    ...
                    setButtonStatus();
                    alert("リクエスト例外、カスタマーサービスに連絡してください");
                    $("#confirm-btn").html('<span class="gui gui-check-square-o"></span> 転送を確認する');
                }
            },
            complete: function () {
    ...
    function tryAgain(data){
        var dialog =  layer.open({
            content:'非常に申し訳ありませんが、異常なネットワーク接続が原因で、この注文のタイムアウトが発生しました。',
            title:'注文はタイムアウトしました',
            skin:'layui-layer-brand',
            area:['360px'],
            btn:['再度試してください','キャンセル'],
...
//        }
        ...
        $.ajax({
            ...
            success:function(data){
                if(data.walletBalance){
                    ...
                    alert("リフレッシュに失敗し、ウォレットの残高が取得されませんでした");
                }
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.apiMoney){
                    ...
                    alert("リフレッシュに失敗し、API残高が取得されませんでした");
                }
        ...
        $.ajax({
            ...
            error: function(error) {
                if (error.status === 600) {
                    ...
                    closeIframeAlert("ゲームは一時的にログインできません。しばらくしてからもう一度お試しください！");
    ...
    function gameFullScreen(){
        ...
        if(ie){// 如果是ie用新开窗口的模式模拟全屏
            window.open(document.location+"?fullscreen", 'ゲーム', 'fullscreen');
        }else{// 如果不是ie正常模式全屏
    ...
    function launchFullscreen(element) {// 启用全屏
        ...
        // 提示使用左上角的退出来退出全屏
        layer.msg('左上隅のアイコンをクリックするか、ESCキーを押して全画面を終了します。',{time:3000});
    }
