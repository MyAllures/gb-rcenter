...
<script>
    ...
    function promoCheck() {
        ...
        $("._vr_promo_check").each(function () {
            ...
            if (nowTime < sTime) {
                ...
                $this.find("._vr_promo_over").hide();
                $this.find("._vr_promo_join").text("今すぐ加入");
    ...
    function changeApplyStatus() {
        ...
        backwaterObj.each(function (i, obj) {
            $(obj).find("._vr_promo_join").text("参加中");
        });
        $.ajax({
            ...
            if (($(actObj).data("code") == "first_deposit" || $(actObj).data("code") == "deposit_send") && flag) {
                if (data.length < 1) {
                    if ($(actObj).data("rankId") === "all") {
                        $(actObj).find("._vr_promo_join").removeClass(oldClass).addClass(newClass + " disabled").text("チャージ時に申請");
                    } else {
                        $(actObj).find("._vr_promo_join").removeClass(oldClass).addClass(newClass + " disabled notfit").text("条件未達");
                    }
                } else {
                    var isContain = false;
