...
<script>
    ...
    function validateCellPhone(obj) {
        ...
        if(!phone){
            BootstrapDialog.alert({message:'携帯番号をご入力ください。',title:'ヒント'});
            return;
        }else if($phone.parents(".form-group").hasClass("has-error")){
            BootstrapDialog.alert({message:'携帯番号を正しく入力ください。',title:'ヒント'});
            return;
        }else if(cookie){
            BootstrapDialog.alert({message:'送信中！',title:'ヒント'});
            return;
        }
                        ...
                        phoneCheckCountBackTimer = setInterval(
                                function(){
                                    ...
                                    if(!sendPhoneIntervalSec){
                                        ...
                                        $this.text(sendPhoneIntervalSec+"秒後に再度送信")
                                    }
...
//                        $this.removeAttr("disabled").text("提交注册");
    ...
    function validateEmailAddress(obj){
        ...
        cookie = Number(cookie);
        if(!email){
            BootstrapDialog.alert({message:'メールアドレスをご入力ください！',title:'ヒント'});
            return;
        }else if($email.parents(".form-group").hasClass("has-error")){
            BootstrapDialog.alert({message:'メールアドレスが正しくありません！',title:'ヒント'});
            return;
        }else if(cookie){
            BootstrapDialog.alert({message:'送信中！',title:'ヒント'});
            return;
