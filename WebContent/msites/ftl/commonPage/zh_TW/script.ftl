    ...
    function AddFavorite(url,title) {
        ...
        try {
            ...
            try {
                ...
                alert("抱歉，您所使用的瀏覽器無法完成此操作，請使用Ctrl+D進行新增");
            }
        }
    }
    ...
    * */
    ...
    function alert(message){
       /* BootstrapDialog.alert({
        ...
        layerDialogNormal(message,'提示資訊','layui-layer-brand',['360px']);
    }
    ...
    <#if imgSrc??>
        if(!localStorage.getItem("${updateTime}"+"-close-home-dialog")){// 判读缓存里是否关闭了首页弹窗
            ...
            <#elseif content?has_content>
                layerDialogIndex('<div style="padding:10px;">${content?replace('\n','')?replace('\r','')}</div><div class="checkbox-wrap"><input type="checkbox" id="home-dialog-checkbox" />關閉後，不再顯示本彈窗廣告</div>','${imgTitl}','layui-layer-brand',['600px'],'','r',true);
    ...
    function handleMt($handle){
        ...
        $handle.find("._vr_mt_slogan").text("維護中");
        $handle.find("._vr_mt_ptSlogan").text("(維護中)");
    ...
    function maintainInfo(st, et,apiId,gameName){
        ...
        if(gameName == 'undefined' || typeof gameName == 'undefined'){
            layerDialogNormal("<div>尊敬的客戶您好：</div><div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+apiName+" 平臺將於"+transTimeZone(sessionStorage.getItem("timezone"))+" "+sTime+" - "+eTime+"進行維護，維護時間若有變動將另行通知。 給您帶來的不便，請您諒解！</div>",'提示資訊','layui-layer-warning',['360px']);
            /*BootstrapDialog.alert({
                message:"",
                title:'提示信息',
            ...
            layerDialogNormal("<div>尊敬的客戶您好：</div><div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+gameName+" 遊戲將於"+transTimeZone(sessionStorage.getItem("timezone"))+" "+sTime+" - "+eTime+"進行維護，維護時間若有變動將另行通知。 給您帶來的不便，請您諒解！</div>",'提示資訊','layui-layer-warning',['360px']);
            /*  BootstrapDialog.alert({
    ...
    function balanceRefresh(){
        ...
        $("._vr_balanceBox").mouseenter(function(){
            ...
            if(isAutoPay == 'true') {
                $("li[name='apiPreNode']").html('<a href="javascript:" onclick="recoveryApi()"><span class="icon-back-m"></span>一鍵回收</a>');
            } else {
                $("li[name='apiPreNode']").html('<a href="javascript:" onclick="getApiBalance()"><span class="pull-right gui gui-refresh"></span>重新整理餘額</a>')
            }
                ...
                if (data.loginSuccess) {
                    ...
                    if (result.defaultLink) {
                        ...
                        if(protocol.indexOf("https:")>-1){
                            ...
                            /*http协议的请求*/
                            if(apiId =="10"){//BBIN 跳转特殊处理 跳转会不对应游戏类型
                                window.location=result.defaultLink;
                                return;
                            }
  ...
  * */
    function changeBalanceHide(bol){
        if(typeof bol === 'undefined'){
            ...
            $('span.eye').removeClass('gui-eye').addClass('gui-eye-slash').attr('title', '顯示餘額');
            $('li.f_refresh').hide();
            ...
            $('span.eye').removeClass('gui-eye-slash').addClass('gui-eye').attr('title', '隱藏餘額');
            $('li.f_refresh').show();
...
//                    getApiBalance();
            ...
            success:function(data){
                if (data) {
                    if (data.msg) {
                        ...
                        dialogMsg("正在回收所有api資金，請稍候！");
                        autoGetApiBalance(obj);
                    } else if (data.resultStatus) {
                        if (data.resultStatus == 'SUCCESS') {
                            dialogMsg("回收成功，請檢視錢包餘額！");
                            autoGetApiBalance(obj);
                        } else if (data.resultCode == 1) {
                            dialogMsg("回收失敗，失敗狀態碼[" + data.resultStatus + "],請稍候再試！");
                        } else {
                            dialogMsg("正在回收中，請稍候再來檢視！");
                            autoGetApiBalance(obj);
                        }
                    } else {
                        dialogMsg("正在回收中，請稍候再來檢視！");
    ...
    function layerDialogDownload(){
        ...
        layer.tab({
            ...
            move:".layui-layer-title",
            tab: [{
                title: '<div class="tit-wrap"><div class="tit">手機APP下載</div><div class="sub-tit">安卓iOS雙原生APP</div></div>',
...
<div id="download-mobile" style="display:none;">
    <div class="code_item ios">
        ...
        <div class="tit_d">iOS APP下載</div>
        <p>使用蘋果手機瀏覽器掃描二維碼， <br />即可下載APP</p>
    </div>
    <div class="code_item android">
        <div class="qrcode qr2" id="code_android">
        </div>
        <div class="tit_d">安卓APP下載</div>
        <p>使用安卓手機瀏覽器掃描二維碼， <br />即可下載APP</p>
    </div>
</div>
        ...
        <ul class="api-list">
            <li>
                <div class="api-name ag">
                    <div class="nam">AG客戶端</div>
                    <div class="tip">登入請新增字首dawoo_， <br />如賬號：dawoo_user01</div>
                </div>
                <div class="download-btn-group">
                    ...
                    <a href="" class="btn-ios">蘋果APP
                        <div class="app-qr">
                            <img src="${data.configInfo.ftlRootPath}commonPage/images/qrcode-example.png"/>
                        </div>
                    </a>
                    <a href="" class="btn-pc">電腦客戶端</a>
                </div>
            </li>
            <li>
                <div class="api-name pt">
                    <div class="nam">PT客戶端</div>
                    <div class="tip">登入請新增字首dawoo_， <br />如賬號：dawoo_user01</div>
                </div>
                <div class="download-btn-group">
                    <a href="" class="btn-android">安卓APP
...
<div id="index-modal-content" style="display: none;">
    ...
    <div class="checkbox-wrap">
        <input type="checkbox"  />關閉後，不再顯示本彈窗廣告
    </div>
</div>
<!--首页透明弹窗内容&ndash;&gt;
