...
define(['common/BaseListPage', 'bootstrap-dialog', 'site/report/operate/filterBox', 'autocompleter'], function(BaseListPage, BootstrapDialog, fBox) {
    ...
    return BaseListPage.extend({
        ...
        changeCenter: function(e) {
            var $subSysCode = $('[name=subSysCode]');
            $('[name="search.siteId"]').val(e.key);
            $('[name=csiteId]').val($('#site_' + e.key).val());
            if (e.key) {
                $subSysCode.val($('[name="role.company"]').val());
                $('div.master').removeClass('hide');
            } else {
                $subSysCode.val($('[name="role.boss"]').val());
                $('div.master').addClass('hide');
            }
        },
        /** 切换站点 */
        ...
        /** 运营商详情 */
        detailCenter: function(e) {
            $('[name="search.centerId"]').val($(e.currentTarget).parent().parent().find('input[name="centerId"]').val());
        ...
        /** 返回运营商 */
        prevCenter: function(e) {
            $('[name="search.siteId"]').val('');
            $('[name=subSysCode]').val($('[name="role.boss"]').val());
            return true;
        },
        /** 返回站长 */
        ...
        validateData: function (e,opt) {
            ...
            /*var subSysCode = $("[name='subSysCode']").val();
            if(subSysCode){
                opt.target =  opt.target + "?subSysCode="+subSysCode;
            }
            var siteId = $("[name='search.siteId']").val();
            if(siteId){
                if(opt.target.indexOf("?")>-1){
                    opt.target =  opt.target + "&search.siteId="+siteId;
                }else{
                    opt.target =  opt.target + "?search.siteId="+siteId;
                }
            }*/
