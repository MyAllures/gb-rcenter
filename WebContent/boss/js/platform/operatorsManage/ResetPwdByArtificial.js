...
define(['common/BaseEditPage'], function (BaseEditPage) {
    ...
    return BaseEditPage.extend({
        ...
        resetPwd: function (e, option) {
            ...
            var msg;
            if (isOnline=='true') {
                msg = window.top.message.platform['resetPwd.artificial.onlineWarning'].replace('{username}', userName);
            } else {
                msg = window.top.message.platform['resetPwd.artificial.warning'].replace('{username}', userName);
            }
            var _this = this;
            window.top.topPage.showConfirmMessage(msg, function (bol) {
                if (bol) {
                    window.top.topPage.ajax({
                        url: root + '/platform/operatorsManage/saveResetPwd.html',
