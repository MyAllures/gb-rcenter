    ...
    return BaseEditPage.extend({
        ...
        bindEvent: function () {
            ...
            $(this.formSelector).on("click",".copy", function () {
                ...
                var sourceContent=$(".apiTypeNameVal"+sourceLocal).val();
                var targetLocal=$(".current").attr("local");
                $(".apiTypeNameVal"+targetLocal).val(sourceContent);
        ...
        changeApiTypeStatus: function (e, opt) {
            ...
            if(oldVal=="disable"&&status!=oldVal){
                window.top.topPage.showConfirmDynamic(window.top.message.common['gameManage.tipsTitle'],
                    window.top.message.content['gameManage.apiType.setEnableTips'],window.top.message.content['gameMange.enable'],
                ...
                window.top.topPage.showConfirmDynamic(window.top.message.common['gameManage.tipsTitle'],
                    window.top.message.content['gameManage.apiType.setDisableTips'],window.top.message.content['gameManage.stillStop'],
                    window.top.message.setting['common.cancel'],function(result){
                        if(result){
                            window.top.topPage.doAjax(e, opt);
                        }
        ...
        uploadFile: function (e, opt) {
            e.objId = $("#siteApiTypeId").val();
            e.catePath = 'siteApiType';
        ...
        revertDefault:function(e,option) {
            ...
            var apiTypeId = option.apiTypeId;
            window.top.topPage.ajax({
                type:"POST",
                url:root+'/siteApiTypeI18n/revertDefault.html',
                data:{'result.local':local,'result.apiTypeId':apiTypeId},
                dataType:"JSON",
                error:function(data){
                },
                success:function(data){
                    if(data.i18n){
                        $(".apiTypeNameVal" +local).val(data.i18n.name);
                    }else{
