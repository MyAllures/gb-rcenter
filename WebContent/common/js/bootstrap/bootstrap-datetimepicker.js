...
!function ($) {
	...
	var DPGlobal = {
		...
		validParts:/HH?|p|P|mm?|ss?|dd?|M{1,3}|yy(?:yy)?/g,
		nonpunctuation:   /[^ -\/:-@\[-`{-~\t\n\rTZ]+/g,
		...
		parseDate:        function (date, format, language) {
			...
			if (/^\d{4}\-\d{1,2}\-\d{1,2}[T ]\d{1,2}\:\d{1,2}$/.test(date)) {
				format = this.parseFormat('yyyy-MM-dd HH:mm');
			}
			if (/^\d{4}\-\d{1,2}\-\d{1,2}[T ]\d{1,2}\:\d{1,2}\:\d{1,2}[Z]{0,1}$/.test(date)) {
				format = this.parseFormat('yyyy-MM-dd HH:mm:ss');
			}
			...
			var parts = date && date.toString().match(this.nonpunctuation) || [],
				...
				parsed = {},
				setters_order = ['HH', 'H', 'mm', 'm', 'ss', 's', 'yyyy', 'yy', 'MMMM','MMM', 'MM','M', 'dd', 'd'],
				setters_map = {
					HH:   function (d, v) {
				...
				val, filtered, part;
			setters_map['H'] = setters_map['HH'];
			setters_map['M'] = setters_map['MM'];
			setters_map['s'] = setters_map['ss'];
			setters_map['d'] = setters_map['dd'];
		...
		formatDate:       function (date, format, language) {
			...
			var val = {
				...
				M:    date.getMonth() + 1,
				MMM:    dates[language].monthsShort[date.getMonth()],
				MMMM:   dates[language].months[date.getMonth()],
				// day
				//d:    date.getUTCDate(),
