            ...
            for (i = 0; errorList[i]; i++) {
                ...
                if (this.settings.highlight) {
                    if (error.element.errorElements) {
                        ...
                        if (currEleIsHidden) {
                            if (( /radio|checkbox/i ).test(error.element.type) && p[0].tagName == 'LABEL') {
                                pHasErrCls = $(p).parent().hasClass("error");
                                if(!currHasErrCls && !pHasErrCls) {
                                    $(p).parent().addClass("error")
                                }
                            } else if (!currHasErrCls && !pHasErrCls) {//当前ele对象隐藏，且当前无error样式，且父节点也没有error样式。
                                p.addClass("error");
                            }
                        } else {
                            if (pHasErrCls) {
