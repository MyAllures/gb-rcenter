...
(function( factory ) {
    ...
    $.validator.setDefaults({
        ...
        onfocusout: false,/*function(element) {
            // 几选一当表单提交时才验证
        ...
        onkeyup: false,/*function( element, event ) {
    ...
    $.validator.addMethod("integerPrecision", function(value, element, param) {
        ...
        return match;
    }, message.valid["integerPrecision"]);
    ...
    $.validator.addMethod("fractionPrecision", function(value, element, param) {
        ...
        return match;
    }, message.valid["fractionPrecision"]);
    ...
    function getErrorElement(message){
        if (typeof (mui) === 'undefined' || !mui){
            console.error("mui is undefined in jquery.validate.extend.mobile.js");
            return;
        }
        if(message){
            mui.toast(message);
